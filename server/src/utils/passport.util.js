    ...
    new GoogleStrategy(
        ...
        async (req, accessToken, refreshToken, profile, done) => {
            try {
                ...
                return done(null, await userService.saveUser(newUser));
            } catch (verifyErr) {
                done(verifyErr);
            }
        }
...
passport.use("github",
    new GithubStrategy(
        {
            clientID: config.GHUB_CLIENT_ID,
            clientSecret: config.GHUB_CLIENT_SECRET,
            callbackURL: config.GHUB_CALLBACK,
            passReqToCallback: true,
            scope: ['user:email'],
